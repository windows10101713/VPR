...
6. AI 도우미 고도화 (2026-06-15)
    * Assistant 페이지가 규칙 기반 응답에서 실제 AI 호출 구조로 변경됨
    * 무료/저비용 중심 제공자 지원:
       - Ollama (로컬 무료, API Key 불필요)
       - Hugging Face Inference (무료 플랜 + 토큰)
       - OpenRouter Free Models (무료 모델 + API Key)
    * 적용 범위:
       - 아이디어 생성
       - 코드 코치(개선안 생성)
       - 구현 플래너(단계별 설계)
    * 설정은 브라우저 localStorage 키 `vpr-ai-config`에 저장
    * 참고: 프론트엔드 저장 구조이므로 공개 배포 시 API 키 보안 프록시 서버 구성이 권장됨
